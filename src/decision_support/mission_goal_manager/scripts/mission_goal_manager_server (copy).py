...
from harpia_msgs.msg import MissionPlannerAction, MissionPlannerGoal, MissionPlannerFeedback, MissionPlannerResult, Mission, MissionPlannerActionGoal
...
class ActionServer():
	...
	def __init__(self):
		...
		self.uav = Drone()
		self.new_goals    = rospy.Subscriber('/harpia/mission_goal_manager/goal', MissionPlannerActionGoal, self.new_goal_callback)
		self.cancel_cb    = rospy.Subscriber('/harpia/mission_goal_manager/cancel', GoalID, self.still_cancelled)
		self.mission      = None
		self.cancel      = None
   ...
   #    }
		...
		while not call_mission_planning():
			if self.a_server.is_preempt_requested():
				...
				while float(rospy.get_time()) - float(self.cancel.id[-14:]) < 10:
					# print("time: ", rospy.get_time())
					# print("id: ", self.cancel.id[-14:])
					# print()
					self.a_server.publish_feedback(feedback_msg)

				print(self.mission.goals)
				replan1(self.mission ,self.uav)
				# self.a_server.set_accepted()
			...
			if self.uav.battery <= 20:
				...
				replan(self.mission, self.uav, base)
			else:
				replan(data.mission, self.uav, None)
